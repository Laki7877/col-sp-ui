
module.exports = function (grunt) {
  'use strict';
  // Project configuration.
  grunt.initConfig({
    connect: {
      server: {
        options: {
          base: 'public/',
          port: 9009
        }
      }
    },
  	browserify: {
  	  dist: {
  	    files: {
  	      'public/assets/js/bundle.js': ['angular/app.js']
  	    },
  	    options: {
          transform: [require('bulkify'), require('browserify-ngannotate')],
  	    	keepAlive: true,
          watch: false
  	    }
  	  }
  	},
    concurrent: {
      options: {
        logConcurrentOutput: true
      },
      dev: {
        tasks: ['watch:scripts']
      }
    },
    watch: {
      scripts: {
        files: ['angular/**/*', 'templates/angular/**/*.html'],
        tasks: ['default']
      }
    },
    exec: {
      compass: 'compass compile' //Only for those with compass
    },
    ngtemplates: {
      dist: {
        src: 'templates/angular/**/*.html',
        dest: 'angular/template.js',
        options: {
          bootstrap: function(module, script) {
            var content = '/**\n * Generated by grunt-angular-templates \n';
            content += ' */\n';
            content += 'module.exports = ["$templateCache", function($templateCache) {' + script + ' }];';
            return content;
          },
          htmlmin: {
            collapseBooleanAttributes:      true,
            collapseWhitespace:             true,
            removeAttributeQuotes:          true,
            removeComments:                 true,
            removeEmptyAttributes:          true,
            removeRedundantAttributes:      true,
            removeScriptTypeAttributes:     true,
            removeStyleLinkTypeAttributes:  true
          },
          url: function(url) {
            //Strip src url
            return url.replace('templates/angular/', '').replace('.html','');
          }
        }
      },
      umeSelect: {
        src: 'angular/modules/ume-select/templates/**/*.html',
        dest: 'angular/modules/ume-select/template.js',
        options: {
          bootstrap: function(module, script) {
            var content = '/**\n * Generated by grunt-angular-templates \n';
            content += ' */\n';
            content += 'angular.module("umeSelect").run(function($templateCache) {' + script + ' });';
            return content;
          },
          htmlmin: {
            collapseBooleanAttributes:      true,
            collapseWhitespace:             true,
            removeAttributeQuotes:          true,
            removeComments:                 true,
            removeEmptyAttributes:          true,
            removeRedundantAttributes:      true,
            removeScriptTypeAttributes:     true,
            removeStyleLinkTypeAttributes:  true
          },
          url: function(url) {
            //Strip src url
            return url.replace('angular/modules/ume-select/templates/', '').replace('.html','');
          }
        }
      },
      productDetail: {
        src: 'angular/modules/product-detail/templates/**/*.html',
        dest: 'angular/modules/product-detail/template.js',
        options: {
          bootstrap: function(module, script) {
            var content = '/**\n * Generated by grunt-angular-templates \n';
            content += ' */\n';
            content += 'angular.module("productDetail").run(function($templateCache) {' + script + ' });';
            return content;
          },
          htmlmin: {
            collapseBooleanAttributes:      true,
            collapseWhitespace:             true,
            removeAttributeQuotes:          true,
            removeComments:                 true,
            removeEmptyAttributes:          true,
            removeRedundantAttributes:      true,
            removeScriptTypeAttributes:     true,
            removeStyleLinkTypeAttributes:  true
          },
          url: function(url) {
            //Strip src url
            return url.replace('angular/modules/product-detail/templates/', '').replace('.html','');
          }
        }
      },
      nc: {
        src: 'angular/modules/nc/templates/**/*.html',
        dest: 'angular/modules/nc/template.js',
        options: {
          bootstrap: function(module, script) {
            var content = '/**\n * Generated by grunt-angular-templates \n';
            content += ' */\n';
            content += 'angular.module("nc").run(function($templateCache) {' + script + ' });';
            return content;
          },
          htmlmin: {
            collapseBooleanAttributes:      true,
            collapseWhitespace:             true,
            removeAttributeQuotes:          true,
            removeComments:                 true,
            removeEmptyAttributes:          true,
            removeRedundantAttributes:      true,
            removeScriptTypeAttributes:     true,
            removeStyleLinkTypeAttributes:  true
          },
          url: function(url) {
            //Strip src url
            return url.replace('angular/modules/nc/templates/', '').replace('.html','');
          }
        }
      }
    }
  });

  // Load local tasks.
  // grunt.loadTasks('tasks');

  // Load plugins
  require('load-grunt-tasks')(grunt);

  // Default task.
  grunt.registerTask('default', ['ngtemplates','browserify']);
  grunt.registerTask('scripts', ['browserify']);
  grunt.registerTask('template', ['ngtemplates']);
  grunt.registerTask('watchall', ['watch']);
  grunt.registerTask('compass', ['exec:compass']);

};
