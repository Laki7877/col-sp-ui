angular.module('app.share')
	.service('CategoryService', function(config) {
	     ...
	     */
	    this.transformNestedSetToUITree = function(set) {
	        var tree = [];
	     ...
	     */
	    this.createSelectFunc = function(columns, selectEvent) {
	     ...
	     * use in category selection
	     */
	    this.createColumns = function(item, template) {
	        var array = [];
	     ...
	     */
	    this.findByCatId = function(catId, tmp) {
	     ...
	     */
	    this.createCatStringById = function(catId, tree) {
	        if(angular.isArray(tree)) {
	            for(var i = 0; i < tree.length; i++) { 
	                var catString = this.createCatStringById(catId, tree[i]);
	                if(catString != null) {
	                    return catString;
	                }
	            }
	            ...
	            if(tree.CategoryId == catId) {
	                ...
	                for (var i = 0; i < tree.nodes.length; i++) {
	                    var check = this.createCatStringById(catId, tree.nodes[i]);
	                    if(check != null) {
	     ...
	     */
	    this.removeLeafByCatId = function(catId, tree) {
