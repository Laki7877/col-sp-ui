angular.module('app.share')
	.service('GlobalCategoryService', function(common) {
		...
		this.getAllForSeller2 = function(treeArray) {
			var array = [];
		...
		this.getAllForSeller = function(treeArray) {
			var array = [];
			angular.forEach(treeArray, function(item) {
				array.push(item);
			});
			...
			angular.forEach(treeArray, function(item) {
				if (item.Visibility == null || !item.Visibility) {
					...
					if(item.nodes.length == 0) {
						...
						item.nodes = this.getAllForSeller(item.nodes);
						if(item.nodes.length == 0) {
							array.splice(array.indexOf(item),1);
						}
					}
