...
<?php $this->start('page-body') ?>
	<div ng-controller="AdminSumProductInfoCtrl">
		<nc-page-title nc-title="Product Information Summary" icon="fa-user">
	          ...
	          <div nc-template="common/input/form-group-with-label" nc-label="Category"  id="form-category">
	            <ui-select ng-model="formData.GlobalCategoryId" >
		          ...
		          <ui-select multiple ng-model="formData.SellerId" >
		            <ui-select-match placeholder="Select or search seller...">{{$item.Seller}}</ui-select-match>
		            <ui-select-choices repeat="opt in sellers  | filter: $select.search">
		              {{ opt.Seller }}
		            </ui-select-choices>
		          </ui-select>
			      </div>
    ...
    <table class="table table-curved">
      <thead>
        ...
        <tr class="table-head">
          ...
          <th nc-sort="Seller">Seller</th>
			    ...
			    <th colspan="3" scope="colgroup">Image</th>
			    <th colspan="3" scope="colgroup">Variation</th>
			    <th colspan="3" scope="colgroup">More</th>
			    <th colspan="3" scope="colgroup">Ready For Action</th>
			    <th>Total</th>
        ...
        <tr>
        	...
        	<th></th>
        	<th></th>
        	<th scope="col"><i class="fa fa-circle" style="color: green;"></th>
        	<th scope="col"><i class="fa fa-circle" style="color: yellow;"></th>
        	<th scope="col"><i class="fa fa-circle" style="color: red;"></th>
        	<th scope="col"><i class="fa fa-circle" style="color: green;"></th>
        	<th scope="col"><i class="fa fa-circle" style="color: yellow;"></th>
        	<th scope="col"><i class="fa fa-circle" style="color: red;"></th>
        	<th scope="col"><i class="fa fa-circle" style="color: green;"></th>
      ...
      <tbody>
        <tr ng-repeat="row in list.data" ng-show="list.data.length > '0'">
          ...
          <td class="width_120">{{row.CreateDate}}</td>
					<td class="width_120">{{row.Category}}</td>
					<td class="width_120">{{row.Seller}}</td>
					...
					<td style="width:50px;">{{row.status}}</td>
					<td style="width:50px;">{{row.status}}</td>
					<td class="width_120">{{row.total}}</td>
