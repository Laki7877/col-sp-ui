...
<div ng-controller="ProductGroupCollectionListCtrl" ng-init="init(<?= $params ?>)">
    ...
    <div class="table-section">
      ...
      <table ng-show="productGroupList.length > 0" class="table table-curved">
        <thead>
          <tr class="table-head" >
        ...
        <tbody>
      	  ...
      	  <tr ng-repeat="row in productGroupList" >		  
                  <td class="checkbox-column">                   
                    <input type="checkbox" aria-label="Checkbox for following text input" ng-model="checkBoxCache[row.CMSId]" />                    
                    <input type="hidden" ng-init="hidCMSTypeId[row.CMSCollectionGroupId] = row.CMSTypeId" ng-model="hidCMSTypeId[row.CMSId]" />
                  </td>                  
				  <td class="column-text-ellipsis"><a href="/collections/{{ row.CMSCollectionGroupId }}">{{ row.CMSNameEN || '(Untitled Collection)' }}</a></td>
