            ...
            <div class="category-section" ng-class="{'column-{{<?=$template?>.length}}' : true }">
                <div class="category-section-border-box">
                    ...
                    <div class="category-content no-padding">
                        <ul ng-repeat="column in <?= $template ?> track by $index" ng-class="{'empty-column': column.list.length <= 0 }" class="content-column">
                            <li ng-repeat="row in column.list track by $index" ng-class="{'category-active' : $index == column.active }" ng-click="$emit('selectGlobalCat', <?= $ng_model ?>, row, $index, $parent.$index)" ng-cloak>{{row.NameEn}}</li>
                        </ul>
                    </div>
                </div>
                <div class="category-footer no-padding">
                    ...
                    <span class="float-right">
                        ...
                        <button type="button" class="btn btn-blue btn-width-xl" ng-click="$emit('saveGlobalCat', <?= $ng_model ?>)" ng-class="{'disabled' : !<?= $ng_model?> }">Select</button>
                    </span>
                </div>
            </div>
      </div>
