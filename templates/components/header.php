      ...
      <li class="dropdown header-font-detail">
        ...
        <li class="dropdown header-font-detail" ng-if="!Imposter && !Profile.User.IsAdmin">
          ...
          <ul class="dropdown-menu" ng-if="Profile.Shop">
            <li><a href="#">Status: <span class="{{asShopStatus(Profile.Shop.Status).color}}">{{asShopStatus(Profile.Shop.Status).name}}</span></a></li>
...
<?php $this->insert('components/modal-change-password', ['id' => 'change-password-modal', 'header' => 'Change Password']) ?>
